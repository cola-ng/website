...
use crate::db::{create_pool, with_conn, DbPool};
use crate::models::{
...
fn get_state(depot: &Depot) -> Result<Arc<AppState>, StatusError> {
    depot
        .get::<Arc<AppState>>("state")
        .cloned()
...
async fn register(
    ...
    let access_token = crate::auth::issue_access_token(
        user.id,
...
async fn auth_required(
    ...
    let claims = crate::auth::decode_access_token(token, &state.config.jwt_secret)
...
#[handler]
async fn me(depot: &mut Depot, res: &mut Response) -> Result<(), StatusError> {
    let state = get_state(depot)?;
    let pool = state.pool.clone();
...
async fn list_records(
    ...
    let records: Vec<LearningRecord> = with_conn(pool, move |conn| {
        use crate::schema::learning_records::dsl::*;
        learning_records
...
async fn create_desktop_code(
    ...
    let code = crate::auth::random_desktop_code();
    let code_hash = crate::auth::hash_desktop_code(&code);
    ...
    let _saved: DesktopAuthCode = with_conn(pool, move |conn| {
        use crate::schema::desktop_auth_codes::dsl::*;
...
async fn consume_desktop_code(
    ...
    let state = get_state(depot)?;
    let pool = state.pool.clone();
    let code_hash_value = crate::auth::hash_desktop_code(&input.code);
...
async fn main() {
    ...
    let router = Router::new().hoop(StateHoop { state }).hoop(cors).push(api);
